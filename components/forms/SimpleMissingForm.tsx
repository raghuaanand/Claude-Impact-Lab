...
interface FormStep {
  name: string;
  value: string;
  type: 'text' | 'number' | 'select' | 'textarea' | 'image' | 'voice';
  label: string;
  icon: string;
  placeholder?: string;
...
export default function SimpleMissingForm({ language }: { language: string }) {
  ...
  const steps: FormStep[] = [
    ...
    },
    {
      ...
      icon: '👕',
      placeholder: 'Clothing, hair, marks, etc.',
    },
    {
      name: 'city',
      value: formData.location.city,
      type: 'text',
      label: 'Last seen where? (City)',
      icon: '🏙️',
    ...
    },
    {
      ...
      type: 'select',
      label: 'I agree to share this info to find them?',
      icon: '✅',
      options: [
        { value: 'no', label: 'No' },
        { value: 'yes', label: 'Yes, help find them' },
      ],
      required: true,
    },
  ];
  ...
  const handleInputChange = (value: string) => {
    ...
    } else if (currentStep.name.startsWith('location')) {
      const field = currentStep.name;
      setFormData((prev) => ({
        ...prev,
    ...
    <div className="flex flex-col h-full px-6 py-8">
      ...
      <div className="mb-8">
        <div className="mb-2 h-2 rounded-full bg-gray-200">
          <div
            className="h-full rounded-full bg-khummela-primary transition-all"
        ...
        <div className="mb-8">
          {currentStep.type === 'text' && (
            <input
              ...
              className="w-full rounded-2xl border-2 border-khummela-primary px-6 py-4 text-2xl text-khummela-text placeholder-khummela-muted focus:outline-none"
              autoFocus
          ...
          {currentStep.type === 'textarea' && (
            <textarea
              ...
              placeholder={currentStep.placeholder}
              rows={4}
              className="w-full rounded-2xl border-2 border-khummela-primary px-6 py-4 text-xl text-khummela-text placeholder-khummela-muted focus:outline-none resize-none"
              autoFocus
            />
          )}
                ...
                <button
                  ...
                  className={`rounded-2xl px-6 py-4 text-lg font-bold transition-all active:scale-95 ${
                    currentStep.value === option.value
                      ? 'bg-khummela-primary text-white shadow-lg'
                      : 'border-2 border-khummela-primary bg-white text-khummela-primary'
                  }`}
                >
                  {option.label}
                </button>
      ...
      <div className="flex gap-4">
        <button
          ...
          className="flex-1 rounded-2xl border-2 border-khummela-primary px-6 py-4 text-2xl font-bold text-khummela-primary hover:bg-khummela-surface disabled:opacity-50 transition-all active:scale-95"
        >
          ← Back
        </button>
        ...
        <button
          onClick={handleNext}
          disabled={loading}
          className="flex-1 rounded-2xl bg-khummela-primary px-6 py-4 text-2xl font-bold text-white hover:bg-khummela-primary-dark disabled:opacity-50 transition-all active:scale-95"
        >
